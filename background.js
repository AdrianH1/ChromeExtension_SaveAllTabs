...
function main(){
    ...
    function folders(tree){
        ...
        console.log("folderid old: " + FolderID);
        ...
        function getWindowIDs(tabs){
            ...
            for (i = 0; i < FolderID.length; i++){
                let removeID = FolderID[i];
                chrome.bookmarks.removeTree(removeID);
            }

            for (i = 1; i <= WindowID.length; i++){
                let paramsCreate = {
                parentId: "2",
            ...
            //---
            chrome.bookmarks.getChildren(parentId, foldersNew);
            function foldersNew(tree){
